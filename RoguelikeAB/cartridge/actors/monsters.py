...
def new_monster(gpos, mtype):
    ...
    def on_player_movement(this, ev):
        if ev.pos == this.pos:  # monster plays after player, so he can get killed 1st
            ...
            pyv.actors.trigger('change_hp', glvars.ref_player, -10)
            return
        blockmap = pyv.actors.peek(glvars.ref_maze).content.blocking_map
        ...
        if pathfinding_result:
            ...
            if len(pathfinding_result) == 2:
                # bash player
                pyv.actors.trigger('change_hp', glvars.ref_player, -35)
    ...
    def on_draw(this, ev):
        ...
        if not pyv.actors.trigger('test', glvars.ref_visibility_mger, mob_position):
    ...
    return pyv.actors.new_actor('monster', locals())
