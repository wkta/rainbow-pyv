...
_defs.vernum to read the version information
...
def init(pygame_pym, gfxmode_str, caption=None, maxfps=60):
    ...
    if not cgmconf.runs_in_web():
        ...
        import katagames_sdk.capsule.engine_ground.overlay as overlay
        print('<->context: Web')
        manager_4web = overlay.upgrade_evt_manager(pygame_pym)
        kevent.gl_unique_manager = manager_4web  # replace ev. manager
...
def cleanup(pygame_pym):
    ...
    if not cgmconf.runs_in_web():
        ...
        print('the update loop is now running forever... (web ctx)')
