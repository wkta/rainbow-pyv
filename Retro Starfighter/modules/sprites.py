...
class Player(pygame.sprite.Sprite):
    def __init__(self, images, bullet_image, P_Prefs):
        ...
        self.weak_bullet_timer = get_ticks()
    ...
    def _flash(self):
        if self.is_hurt:
            ...
            points = pygame.mask.from_surface(
                self.images[self.imgdict_key][f"LV{self.gun_level}"][self.orientation_][self.current_frame]).outline()
    ...
    def _move(self, keyspressed):
        thr=0.376
        cond_up = (keyspressed[self.P_Prefs.key_up]) or (glvars.jhandler[0].hat[0] == (1, 0))
        if glvars.jhandler[0].axis[0] > thr:
            cond_up=True

        cond_down = (keyspressed[self.P_Prefs.key_down]) or (glvars.jhandler[0].hat[0] == (-1, 0))
        if glvars.jhandler[0].axis[0] < -thr:
            cond_down=True
            
        cond_left = (keyspressed[self.P_Prefs.key_left]) or (glvars.jhandler[0].hat[0] == (0, 1))
        # check axe haut-bas sur le stick Left 
        if glvars.jhandler[0].axis[1] < -thr:
            cond_left=True
        cond_right = (keyspressed[self.P_Prefs.key_right]) or (glvars.jhandler[0].hat[0] == (0, -1))
        if glvars.jhandler[0].axis[1] > thr:
            cond_right=True
           
        # IMPROVED JOYSTICK DETECTION (8-dir)
        if glvars.jhandler[0].hat[0] == (-1, -1):
            cond_right = cond_down = True
        if glvars.jhandler[0].hat[0] == (1, 1):
            cond_up = cond_left = True
        if glvars.jhandler[0].hat[0] == (1, -1):
            cond_up = cond_right = True
        if glvars.jhandler[0].hat[0] == (-1, 1):
            cond_down = cond_left = True
    ...
    def _shoot(self, keyspressed):
        ...
        if now - self.shoot_timer > self.shoot_delay:
            ...
            fire_condition = keyspressed[self.P_Prefs.key_fire]
            fire_condition = fire_condition or (glvars.jhandler[0].button[0] == 1)  # joystick (A)
            if fire_condition:
    ...
    def _attack2(self):
        b1 = PlayerBullet(self.bullet_image, Vec2(self.rect.centerx - 10, self.rect.top + 12),
                          Vec2(0, -self.BULLET_SPEED))
        b2 = PlayerBullet(self.bullet_image, Vec2(self.rect.centerx + 10, self.rect.top + 12),
                          Vec2(0, -self.BULLET_SPEED))
    ...
    def _attack3(self):
        b1 = PlayerBullet(self.bullet_image, Vec2(self.rect.centerx - 10, self.rect.top + 12),
                          Vec2(-50, -self.BULLET_SPEED))
        b2 = PlayerBullet(self.bullet_image, Vec2(self.rect.centerx, self.rect.top + 12), Vec2(0, -self.BULLET_SPEED))
        b3 = PlayerBullet(self.bullet_image, Vec2(self.rect.centerx + 10, self.rect.top + 12),
                          Vec2(50, -self.BULLET_SPEED))
        all_sprites_g.add(b1)
        all_sprites_g.add(b2)
...
class FattyBullet(pygame.sprite.Sprite):
    ...
    def update(self, dt):
        if DEBUG_MODE:
            pygame.draw.circle(self.image, "white", (self.image.get_width() / 2, self.image.get_height() / 2),
                               self.radius, 2)
...
class Hellfighter(pygame.sprite.Sprite):
    def __init__(self, images, bullet_img, position, player, g_diff):
        ...
        self.ch_behavior_timer = get_ticks()
