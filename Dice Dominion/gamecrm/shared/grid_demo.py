...
def main():
    ...
    while running:
        ...
        # an Locator drawing)
        xpos, ypos = pos
        xpos = ((xpos-grid.offset_x)/grid._cell_width)*grid._cell_width + \
                                                                grid.offset_x
        ypos = (ypos-grid.offset_y)/grid._cell_height*grid._cell_height+\
                                                                grid.offset_y
        
        # draw Locator
        if show_locator:
            yrow = (ypos-grid.offset_y)/grid._cell_height
            if yrow&1: # odd row
