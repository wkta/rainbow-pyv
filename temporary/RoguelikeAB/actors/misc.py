...
def new_gui():
	...
	def on_player_death(this, ev):
		...
		this.active = True
		ft = pyv.new_font_obj(None, glvars.font_size)  # large txt
	...
	return pyv.actors.new_actor('gui', locals())
...
def new_potion(gpos):
	...
	def on_draw(this, ev):
		...
		if not pyv.actors.trigger('test', glvars.ref_visibility_mger, item_position):
	...
	def on_player_movement(this, ev):
		if ev.pos[0] == this.pos[0] and ev.pos[1] == this.pos[1]:
			pyv.actors.trigger('change_hp', glvars.ref_player, this.effect)
			# trick: an actor can destroy itself
			auto_id = pyv.actors.id_actor(this)
			pyv.actors.del_actor(auto_id)
			pyv.post_ev('item_destroyed', id=auto_id)

	return pyv.actors.new_actor('potion', locals())
...
def new_exit_entity(pos):
	...
	def on_draw(this, ev):
		if not pyv.actors.trigger('test', glvars.ref_visibility_mger, (this.x, this.y)):
			return  # avoid drawing invisble things
		ev.screen.blit(
	...
	return pyv.actors.new_actor('exit_entity', locals())
